...
public class ThemeIntoManager {
    ...
    public static int getParentId(int id) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            int parentId = cursor.getInt(cursor.getColumnIndexOrThrow(SQLiteDatabaseAdapter.THEME_INTO_THEME_ID));
            cursor.close();
            return parentId;
        }
        throw new NoSuchElementException("Missing ThemeIn with id " + id);
    }
